...
if(isset($_POST['simpan'])) { // untuk create
    ...
    if($nim && $namamhs && $jk && $alamat && $kota && $email && $foto) {
        if($op == 'ubah'){ // untuk update
            $sql1   = "update tbl_mhs set nim = '$nim',namamhs = '$namamhs',jk = '$jk', alamat = '$alamat', kota = '$kota', email = '$email', foto = '$
            ' where id = '$id'";
            $q1     = mysqli_query($koneksi, $sql1);
            if($q1){
                    ...
                    <thead>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td scope="row">
                                    <a href="index.php?op=ubah&id=<?php echo $id?>"><button type="button" class="btn btn-danger">Ubah</button></a>
                                    <a href="index.php?op=delete&id=<?php echo $id ?>" onclick="return confirm('Apakah anda yakin ingin delete data?')"><button type="button" class="btn btn-success">Delete</button></a>  
                                </td>
                            </tr>
                        <?php
                        }
